...
<template>
  <div class="grid w-full gap-4 lg:grid-cols-2">
    <div class="w-full">
      ...
      <input id="first-name" v-model="billing.firstName" placeholder="Fatou" autocomplete="given-name" type="text" required />
    </div>

    <div class="w-full">
      <label for="last-name">{{ $t('messages.billing.lastName') }}</label>
      <input id="last-name" v-model="billing.lastName" placeholder="Ndiaye" autocomplete="family-name" type="text" required />
    </div>

    <div v-if="isBillingAddressEnabled" class="w-full col-span-full">
      <label for="address1">{{ $t('messages.billing.address1') }}</label>
      <input id="address1" v-model="billing.address1" placeholder="Ouakam" autocomplete="street-address" type="text" required />
    </div>
    ...
    <div v-if="isBillingAddressEnabled" class="w-full">
      ...
      <input id="city" v-model="billing.city" placeholder="Dakar" autocomplete="locality" type="text" required />
    </div>
    ...
    <div v-if="isBillingAddressEnabled" class="w-full">
      ...
      <input id="zip" v-model="billing.postcode" placeholder="17000" autocomplete="postal-code" type="text" required />
    </div>

    <div v-if="isBillingAddressEnabled" class="w-full col-span-full">
      <label for="phone">{{ $t('messages.billing.phone') }} ({{ $t('messages.general.optional') }})</label>
      <input id="phone" v-model="billing.phone" placeholder="+221 770925131" autocomplete="tel" type="tel" />
    </div>
  </div>
</template>
